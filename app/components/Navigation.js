      ...
      <nav className="mx-auto flex w-full items-center justify-between p-6 lg:px-8" aria-label="Global">
        ...
        <div className="flex lg:flex-1 gap-4 items-center">
          <Link href="/" className="-m-1.5">
            <Image src="/arise-deborah-logo.jpg" alt="Arise Deborah" width={50} height={50} className="rounded-full h-14 w-14" />
          </Link>
          <span className="text-2xl font-bold text-tertiary hidden md:block">Arise Deborah International</span>
        </div>
          ...
          {navigation.map((item) => (
            <Link
              ...
              className={`text-sm font-semibold leading-6 transition-colors duration-200 ${
                pathname === item.href ? "text-primary" : "text-accent hover:text-primary"
              }`}
            >
              {item.name}
            </Link>
          ...
          <>
            ...
            >
              ...
              <div className="flex items-center justify-between w-full">
                <Link href="/" className="text-2xl font-bold text-tertiary" onClick={() => setIsMenuOpen(false)}>
                  Arise Deborah
                </Link>
                <button type="button" className="p-2.5 text-accent" onClick={() => setIsMenuOpen(false)}>
                  <svg className="h-6 w-6" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1.5" fill="none">
                    <path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" />
                  </svg>
                </button>
                ...
                {navigation.map((item) => (
                  <Link
                    ...
                    className={`block rounded-lg px-3 py-2 text-base font-semibold leading-7 ${
                      pathname === item.href ? "bg-primary/10 text-primary" : "text-accent hover:bg-primary/5 hover:text-primary"
                    }`}
                    onClick={() => setIsMenuOpen(false)}
                  >
                    {item.name}
